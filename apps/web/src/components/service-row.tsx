...
export function ServiceRow({ service }: { service: ServiceHealth }) {
  ...
  return (
    <div className="border-border flex items-baseline gap-3 border-b px-4 py-2.5 last:border-b-0">
      <Icon
        className={cn('size-4 shrink-0 translate-y-0.5', STATUS_COLOR[service.status])}
        aria-hidden
      />
      <span className="text-foreground w-24 shrink-0 font-mono text-sm">{service.name}</span>
